...
// #ifndef UNI-APP-X
import { ComputedRef, computed, unref, ref } from '@/uni_modules/lime-shared/vue';
...
async function loadJsonData(jsonUrl : string) : Promise<UTSJSONObject> {
	...
	if (isNetworkUrl(jsonUrl)) {
		...
		return new Promise((resolve, reject) => {
			// 方法1：优先使用 uni.request 加载本地文件（推荐方式）
			// uni.request({
			// 	url: jsonUrl,
			// 	success: (res) => {
			// 		// 本地文件加载成功的状态码可能是200或0
			...
			// 	}
			// })
			// 方法2：如果 uni.request 失败，尝试使用 uni.getFileSystemManager
			try {
